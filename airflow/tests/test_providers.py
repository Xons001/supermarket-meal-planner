...
import pytest
from catalog_sync.providers import (
    ExperimentalMercadonaProvider,
    LocalJsonCatalogProvider,
)
...
def test_local_provider_reads_repeatable_fixture(tmp_path: Path):
    ...
    fixture.write_text(
        json.dumps({"supermarketCode": "MERCADONA", "categories": [], "products": []}),
        encoding="utf-8",
    )
    assert (
        LocalJsonCatalogProvider(str(fixture)).fetch_catalog("MERCADONA")["products"]
        == []
    )
...
def test_local_provider_rejects_another_supermarket(tmp_path: Path):
    fixture = tmp_path / "catalog.json"
    fixture.write_text(json.dumps({"supermarketCode": "MERCADONA"}), encoding="utf-8")
    with pytest.raises(ValueError):
        LocalJsonCatalogProvider(str(fixture)).fetch_catalog("LIDL")
...
def test_experimental_provider_is_disabled(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        ExperimentalMercadonaProvider().fetch_catalog("MERCADONA")
...
def test_invalid_item_is_reported_without_aborting_batch(tmp_path: Path):
    ...
    payload.write_text(
        json.dumps(
            {
                "categories": [],
                "products": [{"externalId": "invalid-product", "name": "Incomplete"}],
            }
        ),
        encoding="utf-8",
    )
